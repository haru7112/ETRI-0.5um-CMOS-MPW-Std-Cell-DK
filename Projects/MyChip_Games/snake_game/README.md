...
* 상/하단 선은 화면 전체 폭, 세로 구분선과 오른쪽 끝이 필드를 닫습니다.
  넷 중 어디에 부딪혀도 죽습니다.
* 점수는 왼쪽 16px 칸에 2자리, page 3(y 24..31)에 표시됩니다.
* 먹이는 약 4Hz로 깜빡여 몸통과 구분됩니다.
* **글자는 하나도 없습니다.** 게임오버/타이틀은 **테두리 깜빡임**으로 알립니다
  (OK를 기다리는 중이라는 뜻). 알파벳 14글자를 폰트에서 빼서 0.06mm²를 아꼈고,
  메시지 배치 먹스가 사라져 pixel_gen이 585 → 352 GE가 되었습니다.
  게임오버 화면에는 죽은 자세 그대로의 뱀과 점수가 남습니다.
* blink 위상은 프레임 시작에 래치됩니다. 프레임 한 장을 밀어내는 데 약 25ms가
  걸려서, 자유 진동시키면 테두리가 반만 그려진 프레임이 나옵니다.
...
│   ├── font_rom.v         5x7 숫자 글리프 10개 (점수 전용)
...
면적은 **qflow 자신의 합성 스크립트**로 측정했고, DK 안에 들어 있는 실제
테이프아웃 `pong_pt1` 으로 검증했습니다 (셀 면적 오차 **+1.2%**).
`cd syn && python3 area.py` 를 돌리면 검증 결과를 먼저 출력합니다.

```
코어 창  (핀 라우팅 링 안쪽 441.0~1459.0um)  1018 x 1018 um = 1.036 mm2
utilisation (실측, pong_pt1.def)                              82.3 %
--------------------------------------------------------------------
예산                                                        0.853 mm2 의 셀

4x4 px 32x16, MAXLEN 32 (기본값)                            1.445 mm2  -> 1.69 배
8x8 px 16x8,  MAXLEN 16 (최소)                              1.246 mm2  -> 1.46 배
```

주의: 평범한 `synth -flatten` + 기본 `abc -liberty` 로 재면 실제보다 **16% 작게**
나옵니다(pong_pt1 기준 0.506 vs 실제 0.600 mm²). 낙관적인 숫자에 속지 않도록
`syn/area.py` 는 qflow 스크립트를 고정으로 씁니다.

설계 자체는 처음보다 크게 줄었습니다(같은 잣대로 -53%). 그래도 예산에는 못
들어가므로, 진짜 숫자는 `cd ETRI050 && make synthesize && make place && make route
&& make size` 로 확인하세요. 남은 선택지는 `docs/area.md` 5절에 있습니다.
...
| 파라미터 | 기본 | 설명 |
|---|---|---|
| `CELL_SH` | 2 | 셀 크기 `2^CELL_SH` px. 1=2x2(64x32), 2=4x4(32x16), 3=8x8(16x8) |
| `MAXLEN` | 32 | 몸통 최대 길이. 면적의 가장 큰 축 |
