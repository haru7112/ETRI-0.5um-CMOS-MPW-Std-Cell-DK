...
4x4 px 32x16, MAXLEN 32 (기본값)                            1.289 mm2  -> 1.51 배
8x8 px 16x8,  MAXLEN 16 (최소)                              1.108 mm2  -> 1.30 배
```

처음 2.769 mm²에서 **1.289 mm² (-53%)** 까지 줄였습니다. 그래도 가장 극단적으로
깎은 설정이 예산의 1.30배라 **현 구조로는 28핀 패키지에 들어가지 않습니다.**
측정 근거, ablation 결과, 남은 선택지는 `docs/area.md` 를 보세요.
