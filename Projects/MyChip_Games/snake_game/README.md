...
* 먹이는 **속이 빈 사각형**, 몸통은 꽉 찬 사각형입니다. 깜빡이지 않습니다.
  2x2 px 셀에서는 외곽선을 그릴 자리가 없어 그 설정에서만 깜빡임으로 되돌아갑니다
  (조건이 상수라 다른 설정에서는 합성 시 사라집니다).
...
make food     # 먹이 배치 전수 검사 (LFSR 2047개 상태 전부)
...
`make system` 에는 **버스 글리치 자가복구** 검사도 들어 있습니다. 패널의 쓰기
포인터를 일부러 어긋내고 두 프레임 안에 화면이 스스로 돌아오는지 확인합니다.
SSD1315는 데이터 바이트를 내용과 무관하게 전부 ACK 하므로 프레이밍이 어긋나도
NACK이 나지 않습니다. 그래서 매 프레임 앞에 주소 윈도우 명령을 다시 보냅니다.

`make food` 은 먹이 배치를 **전수 검사**합니다. LFSR 주기가 2047이라 칩이 만들 수
있는 후보가 유한하므로, 전 주기를 돌면서 (1) 필드 밖 칸이 채택되는 일이 없는지,
(2) 재추첨 루프가 유한한지(최장 21클럭 = 840ns), (3) 특정 열에 먹이가 몰리지
않는지를 확인합니다. 세 가지 셀 크기 모두에 대해 돌립니다.
