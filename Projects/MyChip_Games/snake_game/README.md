...
```
코어 창  (핀 라우팅 링 안쪽)  1018 x 1018 um = 1.036 mm2
utilisation (실측)                            82.3 %
--------------------------------------------------------
예산                                        0.853 mm2 의 셀

현재 (2x2 px, MAXLEN 48)                    1.776 mm2  -> 2.08 배
가장 줄인 설정 (8x8 px, MAXLEN 16)          1.328 mm2  -> 1.56 배
```

`i2c_master` + `oled_ctrl`, 즉 "MCU 없이 패널을 직접 굴리는" 부분만으로 예산의
59%를 씁니다. 기능을 최대한 덜어내도 약 1.5 mm² 코어로 추정되어, **현 구조로는
28핀 패키지에 들어가지 않습니다.** 선택지와 절감 항목별 수치는 `docs/area.md`
4절에 정리해 두었습니다.
